...
describe('GraphQl Helpers', () => {
  test('Check getOperationNames Helper', () => {
    const mockRequest = `
    query operationTest1 {
      characters {
        results {
          name
        }
    ...
    const operationNames = getOperationsNames(mockRequest);

    expect(operationNames).toStrictEqual(['operationTest1', 'operationTest2']);
  });

  test('Check isQueryValid Helper', () => {
    const mockCorrectRequest = `
    query operationTest1 {
      characters {
        results {
          name
    ...
    const mockIncorrectRequest = `query operationTest2 {
      characters {
        results {
          name
    }
    ...
    const correctRequestTest = isQueryValid(mockCorrectRequest);
    const incorrectRequestTest = isQueryValid(mockIncorrectRequest);

    expect(correctRequestTest).toBe(true);
    expect(incorrectRequestTest).toBe(false);
  });

  test('Check isJsonValid Helper', () => {
    const mockCorrectJson = `
      {
        "name": "test" 
      }
    `;

    const mockIncorrectJson = `{name = test}`;

    const correctJsonTest = isJsonValid(mockCorrectJson);
    const incorrectJsontTest = isJsonValid(mockIncorrectJson);

    expect(correctJsonTest).toStrictEqual({ name: 'test' });
    expect(incorrectJsontTest).toBe(false);
  });

  test('Check GetFields Helper', () => {
    const mockSchema = {
      __schema: {
        queryType: {
          name: 'Query',
    ...
    expect(
      getFields(buildClientSchema(mockSchema as unknown as IntrospectionQuery)),
